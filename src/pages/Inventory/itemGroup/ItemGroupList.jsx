...
const columns = [
    {
        title: 'Name',
        dataIndex: 'name',
        render: (text, record) => <span>{text} {record?.children?.length && `(${record.children.length} items)`}</span>,
        ...
        <div className="w-100 h-100">
            ...
            >
                <Table
                    ...
                    onRow={(record) => ({
                        onClick: () => record.ischild?navigate(reverse(routes.inventory.items.view,{id:record.key})):navigate(reverse(routes.inventory.itemGroups.view,{id:record.key})),
                    })}
                    columns={columns}
                    dataSource={data}
                    pagination={{
