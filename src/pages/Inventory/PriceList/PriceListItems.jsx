    ...
    const columns = [
                ...
                return (
                    <>
                        <div className='d-flex flex-column'>
                            <span className='text-primary' style={{ cursor: 'pointer' }} onClick={() => navigate(reverse(routes.inventory.priceList.edit, { id: record.key }))}>{record.name}</span>
        ...
        },
        {
            ...
            render: (record) => {
                return (
                    <>
                        {record?.currenty ? record?.currenty : "-"}
                    </>
                )
            }
        },
        ...
        },
        {
            ...
            render: (record) => {
                return (
                    <>
                        {record?.priceing_scheme ? record?.priceing_scheme : "-"}
                    </>
                )
            }
        },
            ...
            render: (record) => {
                return (
                    <div className='d-flex align-items-center gap-2 action-btn'>
                        <span className='text-primary' style={{ cursor: 'pointer' }} onClick={() => navigate(reverse(routes.inventory.priceList.edit, { id: record.key }))}>Edit</span>
                        <LineOutlined rotate={90} />
                        <span className='text-primary' style={{ cursor: 'pointer' }}>{record.active ? "Make as Active" : "Make as inActive"}</span>
                        <LineOutlined rotate={90} />
