...
const PriceListItems = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (item, record) => {
        return (
          <>
            {record?.currency
              ? `${CurrencyEnum[record?.currency]?.code} - ${CurrencyEnum[record?.currency]?.name}`
              : '-'}
          </>
        );
      }
    },
    {
      ...
      dataIndex: '',
      render: (record) => {
        return <span>{`${record?.percentage} % ${record?.percentage_type}`}</span>;
      }
    },
    {
    ...
    },
    {
      ...
      render: (item) => {
        return <>{item ? roundOfSchemaEnum[item] : '-'}</>;
      }
    },
    {
      title: '',
        ...
        return (
          <div className="d-flex align-items-center gap-2 action-btn">
            <span
              className="text-primary"
              style={{ cursor: 'pointer' }}
              onClick={() => navigate(reverse(routes.inventory.priceList.edit, { id: record.id }))}
            >
              Edit
            </span>
